    ...
    if(isset($_POST['searchword']) || $_POST['searchword']!='')
    {
		...
		if ($result==false) echo "<script type='text/javascript'> alert('查询失败！'); </script>";
		   ...
		   {  
		      ...
		      //echo '<table name="table" id="table" style="word-break:break-word" class="text-center table table-striped table-bordered table-condensed">';   
		      ...
		      while($row = mysql_fetch_array($result)) //输出表头
		      {
				 ...
				 $item .= '<td><a class="btn btn-primary btn-xs" href="bitem.php?bookid='.$row['bookid'].'">查看详细</a></td>';
				 $item .= '</tr>';
				 echo $item;
				 $tag=$tag+1;
              }
